"""Build Knowledge workflow context from one backlog batch."""

from __future__ import annotations

from dataclasses import dataclass

from internal.knowledge.batch.models import BatchActionCandidate, BatchPreparation, BatchWorkingSet
...
class BatchContextBuilder:
    """Build one Knowledge context from one backlog working set."""
    ...
    def build_context(self, *, batch_working_set: BatchWorkingSet) -> BatchPreparation:
        ...
        facet_index_key_counts = FacetGovernanceMetrics.collect_facet_index_key_counts(facet_index=inputs.facet_index)
        index_key_counts = FacetGovernanceMetrics.collect_semantic_index_key_counts(semantic_index=inputs.semantic_index)
        index_case_count = FacetGovernanceMetrics.resolve_index_case_count(semantic_index=inputs.semantic_index)
        ...
        promotable_keys = FacetGovernanceMetrics.resolve_promotable_keys(
            existing_keys=existing_keys,
            index_key_counts=index_key_counts,
            index_case_count=index_case_count,
        )
        demotable_keys = FacetGovernanceMetrics.resolve_demotable_keys(
            existing_keys=existing_keys,
            touched_keys=touched_keys,
            semantic_index_key_counts=index_key_counts,
            facet_index_key_counts=facet_index_key_counts,
        )
        coverage_score = FacetGovernanceMetrics.resolve_coverage_score(
