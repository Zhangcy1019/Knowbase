    ...
    def _build_prompt(
        *,
        ...
        instructions = dedent(
            ...
            - 当前 partition facet schema 中定义的字段，是必须逐一检查的稳定维度。
            - 除这些稳定维度外，如果你发现还有明显有价值的语义维度，也可以额外输出新的开放 key。
            ...
            约束：
            - 只提取输入中明确出现或可直接归纳出的内容，不要编造、猜测或用上下文常识填充缺失值。
            - 对当前 schema 中的每一个 facet 字段都要进行判断：有充分证据时输出对应标签；没有足够证据或该维度不适用时，必须输出空数组 []。
            - 空数组 [] 明确表示“当前 case 无法归入该 facet 维度”，不是错误，也不需要用 unknown、其他占位词或猜测值替代。
            - 不要为了让所有 facet 字段都有值而强行分类；宁可返回空数组，也不要产生不可靠的 facet value。
            ...
            - 在检查完所有稳定 facet 字段之后，你可以补充额外的开放语义 key，用于表达当前 schema 尚未正式收敛但对检索有帮助的维度。
            - 额外开放 key 不会自动成为 facet；只有当前 partition schema 中定义的 key 才会进入 facet projection。
