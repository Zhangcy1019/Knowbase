"""Repository for partition semantic indices."""
...
class PartitionSemanticIndexRepository:
    """Persist and load partition semantic indices."""
    ...
    @classmethod
    def from_env(cls) -> "PartitionSemanticIndexRepository":
        client, index_name = build_knowbase_es_client(
            index_env="CIAGENT_KNOWBASE_PARTITION_SEMANTIC_INDEX_INDEX",
            default_index="ci_knowbase_partition_semantic_index_v1",
        )
    ...
    def get_or_create(self, partition_name: str) -> PartitionSemanticIndexDocument:
        ...
        document = PartitionSemanticIndexDocument(
            ...
            semantic_index=PartitionSemanticIndex(partition_name=partition_name),
            created_at=now,
            updated_at=now,
        )
